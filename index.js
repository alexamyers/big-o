...
// ANSWER: No, because it will only ever run until the 4th item in the numbers array. You could simplify the function but that wouldn't change the time complexity. 
function findFourth(numbers) {
return numbers[3]
}
...
// ANSWER: Yes, by just doubling the numbers in the given array "numbers" we can get what we're after instead of creating a second array and using it to get to the desired result.

function getNumbersSquared(numbers) {
let answers = numbers.map(x => x ** 2)
return answers
}
...
// ANSWER: Both functions are dependent on the size/length of the string received. 